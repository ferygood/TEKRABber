...
author:
- name: Yao-Chung Chen
  affiliation: Human Biology and Primate Evolution, Institute of Bioinformatics, Department of Mathematics and Computer Science, Freie Universität Berlin, Berlin, Germany
  eamil: yao-chung.chen@fu-berlin.de
package: TEKRABber
output:
  BiocStyle::html_document: default
vignette: |
  %\VignetteIndexEntry{TEKRABber} %\VignetteEngine{knitr::knitr} %\VignetteEncoding{UTF-8} %\VignetteBuilder{knitr}
---
# Introduction
...
In the first step, we use `orthologScale()` to get ortholog information and calculate the scaling factor between two species. The species name needs to be the abbreviation of scientific species name used in Ensembl. (This step might take some time based on your data size). 
```{r, message=FALSE}
...
To perform downstream analysis, we use `DECorrInputs()` to create input files. This function will create a "./results" directory in your current path to save the input files for next step.
```{r, warning=FALSE}
...
In this step, we need to generate a metadata contain species name (i.e. human and chimpanzee). The row names need to be same as the DE input table and the column name must be **species** (see as the example below). Then we use `DEgeneTE()` to perform DE analysis. The contrastVector parameters need to be a three element vector. First item is "species", second is the reference species name from metadata which in our case is "human", and the last one is the compared species in your metadata which is "chimpanzee". If you are comparing between two species, set parameter expDesign = TRUE as default.
...
Here we use `corrOrthologTE()` to perform correlation estimation comparing each ortholog and TE. This is the most time consuming step. For a quick demonstration, we use a relatively small size data. You can specify the correlation method and adjusted p-value method. Default is Pearson's correlation with FDR. Please name your output using **.csv** extension and it will genearte it in ./results directory. **Note: ** For more quick and specific analysis, you can subset your data in this step to focus on only the genes and TE that you are interested in. 
```{r, warning=FALSE}
# load built-in data
...
TEKRABber provide an app function for you to quickly view your result. You need to provide the differntially expressed gene/TEs result and correlation results. Metadata is the same one you used for DE analysis.
...
In the Expression tag page, (1) you can specify your input gene and TE. The result will show in boxplots with data points in log2 expression level (2) DE analysis result will show in table for statistical information (3) Correlation result will indicate if this selected pairs are significantly correlated and the value of correlation coffeicients.
![](https://user-images.githubusercontent.com/40789913/143572684-12adf891-3f1b-4b9d-b913-94e664abb196.jpeg)
In the Correlation page, you can visualize your data in scatterplot by three ways. (1) Specify the data point and it will turn red in the distrubtion of your result (2) Show the distrubtion of all the data from your correlation results based on their correlation coefficients and adjusted p-value. The blue vertical dased line indicates the boundary of adjusted p-value is 0.05, and the orange one is for adjusetd p-value 0.01 (3) you can click the data points you are interested in and it will list in the table. You can also drag a area to show data points in certain area. 

## Comparing control and treatment samples within the same species
For this case, you need to generate all the input files following the input format. Here we show a toy data of prepared input files including expression counts from 10 control and 10 treatment samples. The format of input data: row names should be gene name or id, and column name is your sample id (please see details below).
```{r}
# load built-in data
data(geneInputDE)
data(teInputDE)
...
### DE analysis
For DE analysis, you use `DEgeneTE()` function and set the parameter expDesign to **FALSE**. You also need to provide a metadata which this time the column name must be **experiment**. See demo below:
...
### Correlation analysis
For quick demo to perform correlation of orthologs and TEs both in control and treatment sample, we use a relatively small input table which only include 10 orthologs and 10 TEs.
```{r, warning=FALSE}
# load built-in data
...
### explore your data using appTEKRABber():
```{r, warning=FALSE, eval=FALSE}
appTEKRABber(
  DEresult = resultDE,
  corrRef = controlCorr,

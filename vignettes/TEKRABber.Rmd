...
```{r qurey biomaRt and calculate scaling factor, message=FALSE, cache=TRUE, dependson="fetchData"}
...
```{r app visualize, warning=FALSE, eval=FALSE}
appTEKRABber(
  DEresult = hmchimpDE,
...
### DE analysis
For DE analysis in the same species, you also use `DEgeneTE()` function but you need to set the parameter expDesign to **FALSE**. You also need to provide a metadata which this time the column name must be **experiment**. See demo below:
...
```{r load built-in data, warning=FALSE}
# load built-in data

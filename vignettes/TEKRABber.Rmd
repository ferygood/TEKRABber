...
In the first step, we use `orthologScale()` to get ortholog information and calculate the scaling factor between two species. The species name needs to be the abbreviation of scientific species name used in Ensembl. (Note: (1)This step queries information using biomaRt and it might need some time or try different mirror due to connection to Ensembl (2)It might take some time to calculate scaling factor based on your data size). 
...
# If you want to query the data and calculate scaling factor, you can run the code below:
...
chimpGeneCorrInput <- assay(speciesCorr$geneCorr[, speciesCorr$geneCorr$Species=="chimpanzee"])
chimpTECorrInput <- assay(speciesCorr$teCorr[, speciesCorr$teCorr$Species=="chimpanzee"])
...
TEKRABber provides an app function for you to quickly view your result. You need to provide the differentially expressed orthologs/TEs result and correlation results. Metadata is the same one you used for DE analysis.
